...
export default async function BlogArticlePage({ params }: Props) {
  ...
  const extractedH2Titles = extractH2Titles(article.content ?? '')
  ...
  const ctaHtml = `
    <aside class="my-8 rounded-lg border border-cyan-500/30 bg-cyan-500/5 px-5 py-4 text-sm">
      <p class="text-zinc-200">
        Quer um diagnóstico gratuito do seu funil?
        <a href="https://wa.me/5531996966686?text=Ol%C3%A1!%20Estou%20lendo%20o%20artigo%20sobre%20${encodeURIComponent(article.title)}%20e%20gostaria%20de%20uma%20consultoria." class="font-semibold text-cyan-400 hover:text-cyan-300 underline underline-offset-2">
          Falar no WhatsApp →
        </a>
      </p>
    </aside>
  ...
  const proseClassName =
    ...
    "prose-p:my-8 prose-p:text-[#e2e8f0] prose-p:leading-[1.7] " +
      ...
      <article className="blog-container px-4 pt-12 pb-14 lg:pt-16 lg:pb-24 relative od-reveal">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-14 items-start">
          ...
          <div className="lg:col-span-8 lg:col-start-1">
            ...
            {/* Share Section */}
            <div className="mt-16 pt-8 border-t border-[#334155]/30 flex items-center justify-end gap-3">
          ...
          <aside className="lg:col-span-4 space-y-8 sticky top-24 hidden lg:block">
            ...
            {extractedH2Titles.length > 0 && (
              <div className="p-6 bg-[#1e293b]/50 backdrop-blur-md rounded-2xl border border-[#334155]/40 shadow-xl relative overflow-hidden">
                ...
                <p className="text-xs text-zinc-500 mb-4">{article.readTime} · {extractedH2Titles.length} seções</p>
                <nav className="space-y-3">
                  {extractedH2Titles.map((title, idx) => (
                    <a
                      key={idx}
                      href={`#section-${idx}`}
