...
export const metadata: Metadata = {
  title: 'Serviços de Marketing Digital em BH',
  description:
  ...
  openGraph: {
    title: 'Serviços de Marketing Digital | On Demand Digital',
    description:
              ...
              <div className="p-6 md:p-8 grid md:grid-cols-2 gap-8">
                ...
                <div>
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <div>
                      <h2 className="font-extralight uppercase tracking-[0.12em] text-2xl text-white leading-tight">
                        {s.titulo}
                      </h2>
                      <p className="text-white/30 text-sm">{s.subtitulo}</p>
                    </div>
                  </div>
                  ...
                  <p className="text-white/40 text-sm leading-relaxed mb-6">{s.paraQuem}</p>

                  <div className="rounded-xl border border-white/[0.08] bg-white/[0.02] px-4 py-3">
                    <p className="text-white/30 text-xs uppercase tracking-widest mb-1">Resultado esperado</p>
                ...
                {/* Direita */}
                <div>
                  <h3 className="text-white/60 font-semibold text-sm uppercase tracking-widest mb-4">
                    O que está incluso
                  </h3>
                  ...
                  <div className="rounded-xl border border-white/[0.08] bg-white/[0.02] px-4 py-3">
                    ...
                    <p className="text-white/60 text-sm">{s.destaque}</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            {processo.map((p, i) => (
              <div key={i} className="relative">
                <div className="rounded-2xl border border-white/[0.06] bg-white/[0.02] p-6 hover:border-white/[0.12] transition-colors h-full">
                  <div className="font-extralight uppercase tracking-[0.12em] text-4xl text-transparent bg-clip-text bg-gradient-to-br from-white to-white/60 mb-3 leading-none">
                    {p.step}
                  </div>
                  <h3 className="font-extralight uppercase tracking-[0.12em] text-white text-lg mb-2">{p.titulo}</h3>
                  <p className="text-white/40 text-sm leading-relaxed">{p.descricao}</p>
                </div>
                {i < processo.length - 1 && (
                  <div className="hidden md:flex absolute top-1/2 -right-3 -translate-y-1/2 z-10 text-white/15 text-lg">
                    →
                  </div>
                )}
              </div>
      ...
      <section className="od-section-lg">
        <div className="od-container text-center max-w-2xl mx-auto">
          ...
          <a
            ...
            className="inline-flex items-center gap-2 px-8 py-4 text-white font-bold rounded-xl shadow-lg hover:opacity-90 transition-opacity text-lg"
